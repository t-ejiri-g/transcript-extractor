...
1. Chromeブラウザで Teams または SharePoint 上の録画・トランスクリプトページを開く
2. トランスクリプトのパネルを表示する（Copilotのライセンスプランなどによってブラウザの表示が違うので、すべてのプランで検証はまだできていません）
3. `Cmd+Shift+S` または `Ctrl+Shift+S` を押す（Macは検証済み、Windowsは非検証）
4. `会議タイトル_YYYY-MM-DD_HH-MM-SS.txt` または `transcript_YYYY-MM-DD_HH-MM-SS.txt` がダウンロードされる

Teams / SharePoint のDOMスクレイピングでは、トランスクリプト一覧を先頭まで自動で戻してから、仮想スクロールされた項目を順に読み込みながら収集します。長い会議では完了まで時間がかかることがあります。
※自動で動いている間は、他の操作はしないようにお願いします。挙動が不安定になります。
...
- `injected-helpers.js`: ページ側で取得したレスポンスのデコード補助
- `injected.js`: ページ側の `fetch` / XHR を監視して字幕データを検出
- `content.js`: 取得データの保存依頼、DOMスクレイピング、ダウンロード処理
- `parser.js`: VTT / Google caption pb3 のパース、DOM由来メタデータの抽出、テキスト整形
- `session.js`: ダウンロード実行単位の `requestId` 管理
- `scroll.js`: 仮想スクロールされたトランスクリプト一覧の操作補助
- `test/`: Jest テスト
...
- Teams / SharePoint 側のUI変更により、DOMスクレイピングが動かなくなる可能性があります。
